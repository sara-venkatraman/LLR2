...
LLR2 <- function(geneData, hours, bayes=TRUE, priorMatrix=NULL, writeToCSV=TRUE) {
  ...
  integrals <- parallel::mclapply(geneDataList, function(y)
  ...
  X <- parallel::mclapply(genePairsAsList, function(pair) do.call(Get.Design.Matrix, pair))

  # Compute the three R^2 values for each pair of genes
  if(bayes == TRUE) {
    Get.Prior.Indicator <- function(idx1, idx2) priorMatrix[idx1, idx2]
    priorList <- parallel::mclapply(genePairsAsList, function(pair) do.call(Get.Prior.Indicator, pair))
    regressionDataList <- Map(list, X, Y, priorList)
    R2 <- parallel::mclapply(regressionDataList, function(x) do.call(Compute.LLR2.Bayes, x))
  } else {
    regressionDataList <- Map(list, X, Y)
    R2 <- parallel::mclapply(regressionDataList, function(x) do.call(Compute.LLR2.OLS, x))
  }
